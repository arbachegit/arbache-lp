...
export async function safeHover(page: Page, selector: string): Promise<boolean> {
  try {
    const element = page.locator(selector).first();
    await expect(element).toBeVisible({ timeout: 5000 });
...
export async function checkElementState(page: Page, selector: string) {
  try {
    const element = page.locator(selector).first();
    return {
    ...
    return { visible: false, enabled: false, editable: false };
  }
}
