...
import { secureFetch, secureLog, generateRequestId, cleanResponse, CurateInputV1, AnswerInputV1, GenerateInputV1, SummarizeInputV1, TextOutputV1, } from '../lib/guardrails.js';
// ===================================
// CONFIG
// ===================================
class OpenAIConfig {
...
`;
async function callGPT(systemPrompt, userMessage, maxTokens, temperature, requestId) {
    ...
    const response = await secureFetch('https://api.openai.com/v1/chat/completions', requestId, {
    ...
    try {
        switch (name) {
            case 'curate_response': {
                ...
                const result = await callGPT(systemPrompt, validated.content, 1024, 0.7, requestId);
...
${ARBACHE_CONTEXT}`;
                const userMessage = validated.search_results
                    ...
                    : `Pergunta: "${validated.question}"`;
                const result = await callGPT(systemPrompt, userMessage, 1024, 0.7, requestId);
                const cleanedText = cleanResponse(result.text);
                const output = TextOutputV1.parse({ text: cleanedText, tokens_used: result.tokens_used });
                secureLog('info', 'Tool completed', requestId, { tool: name, success: true });
                return {
            ...
            case 'generate': {
                ...
                const result = await callGPT(system, validated.prompt, maxTokens, temperature, requestId);
            ...
            case 'summarize': {
                ...
                const result = await callGPT(systemPrompt, validated.text, 500, 0.3, requestId);
                const output = TextOutputV1.parse(result);
                secureLog('info', 'Tool completed', requestId, { tool: name, success: true });
                return {
                    content: [{ type: 'text', text: output.text }],
